...
try save_set = params.eeglab.save.set; catch save_set = true; end
 merge_sets = params.eeglab.merge_sets; %catch merge_sets = false; end
...
for isub = 1:nsub
  ...
  %try
    ...
    try sinfo_idx = strmatch(subid,{sinfor.id}); catch sinfor_idx = []; end
	...
	% Check the sinfo session variable to figure out which sessions to use
	 try use_sessions = sinfor(sinfor_idx).use_session; 
	...
	EEG = pop_biosig(bdffname, ...
	    'rmeventchan', params.bdf.rmeventchan, 'blockrange',[]);
	...
	if ~exist(filtfname,'file') || overwrite_filtered
	  ...
	  for itype = 1:ntypes
	    ...
	    chan_idxs = ...
		find(ismember({EEG.chanlocs.labels},biosemi_chan_labels('eeg')));
...
function params = get_default_params
  ...
  params.eeglab.filter.eeg.low_cutoff = [0.1];
